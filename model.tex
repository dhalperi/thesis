...
As in the example of \secref{sec:eff_snr_example}, the SNR of a $1\times1$ channel matrix is simply the power of the single entry in the CSI matrix. There is one Subchannel SNR for each OFDM subcarrier:
\begin{equation}
...
\end{equation}
for $j \in [1,S]$ and $S\times S$ identity matrix $I_S$~\cite{Tse}.
...
By adapting the conversion between the CSI and the Effective CSI, my model supports a wide variety of application decisions. In this section I explain how to compute the Effective CSI $\mathbf{H}'$ from the CSI $\mathbf{H}$ for some of these problems.
...
\begin{equation}
	\mathbf{H}'_i = \mathbf{V}_i\mathbf{H}_i.
\end{equation}
...
\section{Higher-level Applications}
I describe how to use Effective SNR to implement a number of higher-level applications.

\heading{Channel Selection.} When two wireless devices initiate a peer-to-peer connection, they have a choice of many different 802.11 channels that correspond to many different operating frequencies. To optimize channel selection, the devices can frequency-hop in unison and take CSI measurements of each channel.

\heading{Access Point Selection.} XXX.
