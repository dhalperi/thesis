...
The transmitter need only measure the channel SNR and apply textbook formulas that can compute the error rates of particular modulations. The fastest rate can then be easily selected.
...
My approach to the Wi-Fi network configuration problem is to return to the basic strategy of using theory to connect the performance of 802.11 devices on real links to measurements of their underlying radio channels in practice. My hypothesis is that it is possible to use channel measurements readily available in 802.11n Wi-Fi devices to inform decisions about wireless network device configurations. Rather than using the heuristic and adaptation-based approaches used to configure networks today, I propose a 


 Though this was intractable in the past, there are three reasons for the opportunity to make progress.
\begin{itemize}
